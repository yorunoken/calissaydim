    ...
    return (
        <div className="relative">
            ...
            >
                ...
                <svg className={`w-5 h-5 transition-transform duration-300 ${isOpen ? "transform rotate-180" : ""}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            {isOpen && (
                <ul className="absolute w-full mt-1 max-h-60 overflow-y-auto bg-[#292929] border border-gray-700 rounded-md shadow-lg z-10 transition-all duration-300 ease-in-out scrollbar-thin scrollbar-thumb-[#61dafb] scrollbar-track-[#1e1e1e]">
                    {options.map((rank, index) => (
                        <li key={index} onClick={() => handleSelect(rank)} className="px-4 py-2 hover:bg-[#333333] cursor-pointer transition-colors duration-200 flex items-center">
